...
flood<-read_csv("output/flooded_percent_from_landsat_berm_pixels.csv")

##read in the estimated green up day of year, lst and soil temp data
heat.l8<-read_csv("output/berm_pixels_landsat8_heat_processed.csv")
...
daymet<-read_csv("/home/kyle/Documents/Documents/UT/Sapelo/GEE/daymet/berm/output/daymet_bermplots")

##read in the predictions for the aboveground biophysical models
data<-read_csv(paste0("output/xgb", v, "_predicted_biophysical_seagrant_landsat8.csv"))
...
loc<-unique(data$pix)
dates<-seq.Date(as.Date("2013-04-15"), as.Date("2023-03-01"), by= "1 month")
...
out$site<-ifelse(out$site=="dc", "deanc", out$site)
out$site<-ifelse(out$site=="fr", "folly", out$site)
out$site<-ifelse(out$site=="du", "dupli", out$site)
out$site<-ifelse(out$site=="hc", "huntc", out$site)
out$site<-ifelse(out$site=="ns", "north", out$site)
...
data<-merge(out, elevation, by = "pix")
...
#deletes interpolated data at old berm sites between jessica's and my sampling
field_interpolate <- c(colnames(data)[10:22])
jberm <- c("fluxb", "skida", "ugami")
data[field_interpolate] <- lapply(data[field_interpolate], function(x) replace(x, data$date > as.Date("2016-11-01") & data$date < as.Date("2021-06-01") & data$site %in% jberm, NA))
...
tide<-read.csv("data/fort_pulaski_monthly_mean_sea_level_NAVD_CO-OPS_8670870_wl_2013_2023.csv", stringsAsFactors = F)
...
data <- data[!is.na(data$pix),]
write_csv(data, "output/processed_gt_d.csv")

flood_by_year <- data %>% group_by(pix, year) %>% summarise(mean_local_hiwater = mean(local_hiwater), mean_local_lowater = mean(local_lowater))
flood_by_year$site<-substr(flood_by_year$pix, 1,2)

flood_by_year <- flood_by_year %>% group_by(year, site) %>% summarise(hi = mean(mean_local_hiwater), lo = mean(mean_local_lowater))

ggplot(data = flood_by_year) +
  geom_line(aes(x = as.numeric(year), y = lo, color = site))
...
write_csv(data, file = paste0("/home/kyle/Documents/Documents/UT/Sapelo/berm_output/processed_bg", v, ".0.csv"))
