...
ls <- list.files(path = "/home/kyle/Documents/Documents/UT/Sapelo/berm_output", pattern = "landsat_hladik_split")
Sys.time()
for(i in seq(15,length(ls))){
...
newpixels<-read_csv(paste0("/home/kyle/Documents/Documents/UT/Sapelo/berm_output/xgb_predicted_biophysical_landsat_hladik_split", cut_no, ".csv"))
newpixels$date<-as.Date(newpixels$date)
newpixels <- newpixels %>%
  filter(pix != "471540 3475230" & pix != "474120 3484620")
...
####
Sys.time()
newpixels.daymet<-read_csv("/home/kyle/Documents/Documents/UT/Sapelo/GEE/daymet/gce/output/daymet_monthly_climate_newpixels.csv")
Sys.time()
newpixels.daymet <- newpixels.daymet %>%
  filter(newpixels.daymet$pix %in% newpixels$pix)

###predict hybrid bg for newpixels
##smooth veg data series to estimate info on all valid landsat dates
dates<-seq.Date(as.Date("2014-01-15"), as.Date("2023-12-15"), by = "1 month")
...
Sys.time()
out<-newpixels %>% dplyr::group_by(pix, flood_time) %>% dplyr::filter(length(date)>1) %>% #date>25
  do(data.frame(date=dates,
                ...
                year_obs=c(rep(length(.$date[.$year==2014]), length(dates[format(dates, "%Y")=="2014"]))),
...
newpixels$startn<-NA
newpixels$startlai<-NA
newpixels$startchl<-NA
newpixels$startphoto<-NA


for (j in seq_along(yrs)){
  for (i in seq_along(pixels)){
    if(anyNA(newpixels$greenupmo[newpixels$year==yrs[j] & newpixels$pix==pixels[i]]) == F){
...
dem<-raster("/home/kyle/Documents/Documents/UT/Sapelo/Mapping/ModDEM_ascii/mod_dem.txt")
...
newpixels2<-merge(newpixels, newpixels.daymet, by=c("key", "pix", "utm_east", "utm_north"), all.x=T)

newpix_test <- newpixels2 %>%
  dplyr::select(key,utm_east,utm_north, fid)

if(save_model==T){
  write_csv(newpixels2, paste0("/home/kyle/Documents/Documents/UT/Sapelo/berm_output/xgb_processed_landsat8_hladik_split", cut_no, ".csv"))
}
}
Sys.time()
